...
/* Intervalo entre elementos da sequência ao mostrar (ms) */
...
void game_mostrar_sequencia(jogo_t *jogo) {
    ...
    for (int i = 0; i < jogo->nivel; i++) {
        ...
        led_acender(btn);
        audio_tocar_nota(btn);   /* bloqueia DURACAO_NOTA_MS no Core 1 */
        led_apagar(btn);
...
int game_verificar_entrada(jogo_t *jogo, int btn) {
    ...
    /* Reinicia posição ao entrar num novo nível */
...
void game_feedback_acerto(void) {
    /* Pisca todos os LEDs uma vez rapidinho */
